...
def build_queries(intent_text: str, artist: str | None, song: str | None, genre: str | None, era: str | None) -> list[str]:
    ...
    if artist and song:
        queries.insert(0, f'"{artist}" "{song}" filetype:kar')
...
def extract_kar_candidates(page: PageContent, allowed_domains: list[str] | None = None) -> list[KarCandidate]:
    ...
    for raw_link in raw_links:
        url = urljoin(page.source_url, raw_link.strip().strip("'\""))
...
def is_direct_kar_url(url: str) -> bool:
    parsed = urlparse(url)
    return parsed.scheme in {"http", "https"} and parsed.path.lower().endswith(".kar")
